...
public class WSSolver {
    ...
    public static void main(String[] args) {
    	for (String arg: args) {fileName = arg; break;}
        GetData();
        for (String word : wordsToFind.stream()
        ...
        if(wordsToFind.size() > solution.size()) ErrorFound('F');
        else if(wordsToFind.size() < solution.size()) ErrorFound('G');
    ...
    static void GetData() {
        try {
                ...
                if (count <= dimension) { // if true then we still should be in the word search
                    ...
                    for (char c : line.toCharArray()) {
                        if (Character.isLetter(c) && Character.isUpperCase(c)) { // should be a Capitalized Letter
                            ...
                            ErrorFound((Character.isLetter(c)) ? 'C' : 'A'); // If is letter then it's not Capital, else is a number
                    }
                } else {
                    //Check if the next line is supposed to be from the soup or not, and if yes, then report error. 
            ...
            for(String word: wordsToFind) {
            	...
            	if(somaMaiusculas == word.length()) ErrorFound('B');
            }
    ...
    static void SearchWord(String word, int x, int y, int index, Direction direction, int number, int overlapCounter) {
        if (overlapCounter == word.length()) return;
        if (index >= word.length() && overlapCounter < word.length()) {	//If the word is found and not overlaping then we write the word
